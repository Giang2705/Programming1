...
// day dung k
    ...
    public void readOrderFile() throws IOException{
        try {
            FileReader reader = new FileReader("Database\\DataOrder.csv");
            BufferedReader readingOrder = new BufferedReader(reader);
            readingOrder.readLine();
            String line;
            line = readingOrder.readLine();

//            boolean firstLine = true;
            while (line != null) {
                String[] arrayOrder = line.split(",");
                Order temp = new Order(arrayOrder[0], new Member(arrayOrder[1]), new Product(arrayOrder[2]),)  ;
//                if (array[0] != "id" && array[1] != "name") {
                System.out.printf("ID Order: " + temp.getIdOrder() + ", ID Member: " + temp.getMember()
                        + ", Name Product: " + temp.getProducts() + ", The amount of the products: " + temp.getAmount()
                        + "Time: " + temp.getTime() + "\n");
